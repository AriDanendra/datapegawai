...
const RiwayatOrganisasi = ({ data: propData, employeeId: propEmployeeId }) => {
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      let response;
      if (modalType === 'add') {
        response = await axios.post(`/api/employees/${employeeId}/organisasi`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setOrganisasiData([...organisasiData, response.data]);
        showSuccessModal(`Data organisasi baru berhasil ditambahkan!`);
      } else {
        response = await axios.put(`/api/employees/${employeeId}/organisasi/${selectedData.id}`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`/api/employees/${employeeId}/organisasi/${selectedData.id}`);
