...
const StatusKepegawaian = ({ data: propData, employeeId: propEmployeeId }) => {
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      let response;
      if (modalType === 'add') {
        response = await axios.post(`/api/employees/${employeeId}/status`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setStatusData([...statusData, response.data]);
        showSuccessModal(`Data status baru berhasil ditambahkan!`);
      } else {
        response = await axios.put(`/api/employees/${employeeId}/status/${selectedData.id}`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`/api/employees/${employeeId}/status/${selectedData.id}`);
